...
function addReview(review, bookId) {
  ...
  return getById(bookId).then(({ book, books }) => {
...
function addBook({
  ...
  _saveToStorage(books)
  return Promise.resolve(
    `http://127.0.0.1:5502/index.html#/book-app/${newBook.id}`
  )
}

function getNextBookId(bookId, step) {
  const books = _loadFromStorage()
  let bookIdx = books.findIndex((book) => book.id === bookId)
  bookIdx += step
  if (bookIdx < 0 || bookIdx > books.length - 1) bookIdx -= step
